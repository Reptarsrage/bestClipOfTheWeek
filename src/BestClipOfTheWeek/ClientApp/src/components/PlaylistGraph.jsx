...
export default class PlaylistGraph extends Component {
  ...
  constructor() {
    ...
    this.state = {
      ...
      data: [],
      dimensions: [
        { align: 'right', color: '#3498DB', enabled: true, icon: 'eye', key: 'd1', name: 'Views' },
        { align: 'left', color: '#2ECC71', enabled: true, icon: 'thumbs-up', key: 'd2', name: 'Likes' },
        { align: 'left', color: '#E74C3C', enabled: false, icon: 'thumbs-down', key: 'd3-format', name: 'Dislikes' },
        { align: 'left', color: '#8E44AD', enabled: true, icon: 'comments', key: 'd4', name: 'Comments' },
      ],
      max: 5,
      showAdvanced: false,
    }
  ...
  getViewToolbar() {
    ...
    const inputs = dimensions.map((dim) => {
      ...
      const style = { color }
      const iconElt = <FontAwesomeIcon className="mr-2" icon={['fas', icon]} />
      return (
  ...
  getGraph() {
    ...
    bars =
      bars &&
      bars.map((dim) => <Bar dataKey={dim.key} fill={dim.color} key={dim.key} name={dim.name} yAxisId={dim.align} />)
    return (
      <ResponsiveContainer height="100%" width="100%">
        <BarChart data={data}>
          <XAxis dataKey="name" tick={false} />
    ...
    const showAdvancedButton = showAdvanced ? null : (
      <div className="col-12 col-sm-8 text-left">
        <button type="button" className="btn btn-outline-danger" onClick={this.toggleAdvancedToolbars}>
          <FontAwesomeIcon className="mr-2" icon={['fas', 'filter']} />
          Advanced
        </button>
      </div>
    )
